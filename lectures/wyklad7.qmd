...
> 🔸 Takie podzapytanie **powinno** zwrócić jedną wartość (scalar subquery). Co się
> stanie, gdy zwróci więcej niż jedną, zależy od silnika: **PostgreSQL zgłosi błąd**
> (`more than one row returned by a subquery used as an expression`) — ale
> **SQLite nie zgłasza błędu**, tylko po cichu bierze jeden z wierszy (który
> konkretnie — nie jest gwarantowane) i ignoruje resztę. To niebezpieczne "milczące"
> zachowanie: kod, który w SQLite "działa", może się wywrócić dopiero po
> przeniesieniu na PostgreSQL. Nie polegajcie na konkretnym zachowaniu — jeśli
> podzapytanie może zwrócić więcej niż jeden wiersz, użyjcie `IN` (sekcja niżej) albo
> dodajcie `LIMIT 1` ze świadomym sortowaniem.
